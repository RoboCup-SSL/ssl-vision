...
void VarItemDelegate::drawBar (VarData * dt, QPainter * painter, const QStyleOptionViewItem & option, const QModelIndex & index ) const {
  (void)index;
  ...
  if (index.isValid() && index.model()!=0) {
    ...
    if (item!=0) {
      ...
      if (dt!=0) {
        ...
        if (editor!=0 && ((dt->getRenderFlags() & DT_FLAG_READONLY) != 0x00)) {
