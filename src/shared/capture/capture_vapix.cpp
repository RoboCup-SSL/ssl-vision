...
CaptureVapix::CaptureVapix(VarList *_settings, int default_camera_id, QObject *parent) : QObject(parent), CaptureInterface(_settings) {
  ...
  v_cam_network = new VarString("cam network prefix", default_network_prefix);
  v_cam_ip = new VarString("cam ip", toIpString(v_cam_network->getString(), default_camera_id));
...
string CaptureVapix::toIpString(string network_prefix, int ip) {
    return network_prefix + "." + to_string(ip);
}
...
RawImage CaptureVapix::getFrame() {
  ...
  if (curl_result != CURLE_OK) {
    fprintf(stderr, "VAPIX: Unable to get response from camera: %d with ip %s: %s\n", 
